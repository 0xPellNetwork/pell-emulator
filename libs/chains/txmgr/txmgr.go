...
func NewSimpleTxManager(
	...
	return &SimpleTxManager{
		wallet:             wallet,
		client:             client,
		log:                logger.With("module", "txMgr/SimpleTxManager", "sender", sender.String()),
		sender:             sender,
...
func (m *SimpleTxManager) Send(ctx context.Context, tx *types.Transaction) (*types.Receipt, error) {
	...
	if err != nil {
		m.log.Info("Transaction receipt not found", "err", err)
		return nil, err
	}
...
func (m *SimpleTxManager) queryReceipt(ctx context.Context, txID wallet.TxID) *types.Receipt {
	...
	if errors.Is(err, ethereum.NotFound) {
		...
		m.log.Info("Receipt and error are both nil", "txID", txID)
		return nil
	}
