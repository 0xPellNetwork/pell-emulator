...
function show_operator_registered {
  local ADDRESS=$1
  export STAKING_DELEGATION_MANAGER=$(ssh hardhat "cat $HARDHAT_CONTRACTS_PATH/DelegationManager-Proxy.json" | jq -r .address)
  export PELL_DELEGATION_MNAGER=$(ssh hardhat "cat $HARDHAT_CONTRACTS_PATH/PellDelegationManager-Proxy.json" | jq -r .address)
  export IS_STAKING_OPERATOR=$(cast call $STAKING_DELEGATION_MANAGER "isOperator(address)(bool)" $ADDRESS)
  export IS_PELL_OPERATOR=$(cast call $PELL_DELEGATION_MNAGER "isOperator(address)(bool)" $ADDRESS)
...
function show_dvs_operator_info {
  ...
  OPERATOR_INFO=$(cast call "$DVS_CENTRAL_SCHEDULER" "getOperator(address)((bytes32,uint8))" $OPERATOR_ADDRESS)
  OPERATOR_INFO=$(echo "$OPERATOR_INFO" | tr -d '()' |tr -d ' ')
  echo "Operator info: $OPERATOR_INFO"
  export OPERATOR_ID=$(echo "$OPERATOR_INFO" | cut -d ',' -f 1)
  export OPERATOR_STATUS=$(echo "$OPERATOR_INFO" | cut -d ',' -f 2)
}
